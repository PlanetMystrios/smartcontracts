...
contract Mystrium is Token {
    
    using SafeMath for uint256;
    
    string public name = "Mystrium Ore";
    uint8 public decimals = 18;
    string public symbol = "MYSTRIUM";
...
contract MystrianPeople is ERC721Burnable,Ownable {
    ...
    event addMystrians(address player);
    ...
    mapping (address => Char[]) public charManager;
    address [] public managers;
    
    uint public price =  1 * 10**15;
    uint public mystriumPrc = 100 * 10**18;
    uint public chngPrc = 100 * 10**18;
    uint public trainPrc = 100 * 10**18;
    uint public ratio = 0x2;
    
    constructor(Token _tokenContract) ERC721("Mystrian People", "MYSTRIAN") {
        address msgSender = _msgSender();
        developer = payable(msgSender);
        tokenContract = _tokenContract;
    }
    
    function mystriosRand(uint mod) public view returns(uint) {
    ...
    function addChar(string memory _name,uint _color) external payable {
        ...
        charManager[senderAdr].push(Char(senderAdr,_name,_color));
        managers.push(senderAdr);
    }
    
    function getChar(address _charManager) public view returns (address,string memory,uint) {
        Char storage char = charManager[_charManager][0];
    ...
    function changeChar(string memory _name,uint _color) external payable {
        ...
        developer.transfer(msg.value);
        require(msg.value >= price);
        Char storage char = charManager[senderAdr][0];
        char.name = _name;
        char.color = _color;
    }  
    
    function newMystrian() external payable returns (uint256) {
        address senderAdr = msg.sender;
        developer.transfer(msg.value);
        require(msg.value >= price);
        emit addMystrians(senderAdr);
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        string memory nameNumb = uint2str(mystriosRand(0x270F));
        string memory tokenURI = string(abi.encodePacked("Mystrians-", nameNumb));
        _mint(senderAdr, newItemId);
        _setTokenURI(newItemId, tokenURI);
        string memory _name = tokenURI;
        uint _atx = mystriosRand(0x23);
        uint _def = mystriosRand(0x24);
        uint _str = mystriosRand(0x25);
        uint _no = mystriosRand(0x63);
        uint _body = mystriosRand(0x3);
        uint _eye =  mystriosRand(0x8);
        uint _hair =  mystriosRand(0x2A);
        mystrians.push(Mystrians(_name,_atx,_def,_str,_no,_body,_eye,_hair));
        return newItemId;
    }
    
    function getMystrian(uint _mystrian) public view returns (string memory,uint,uint,uint,uint,uint,uint,uint) {
        Mystrians storage mystrian = mystrians[_mystrian];
        return (mystrian.name,mystrian.atx,mystrian.def,mystrian.str,mystrian.no,mystrian.body,mystrian.eye,mystrian.hair);
    }
    
    function addNewMystrian(address player) internal returns (uint256) {
        emit addMystrians(player);
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        string memory nameNumb = uint2str(mystriosRand(0x270F));
        string memory tokenURI = string(abi.encodePacked("Mystrians-", nameNumb));
    ...
    function changeMystrians(uint256 tokenId, string memory _name,uint _body,uint _eye,uint _hair) internal {
        uint arrayId = tokenId - 1;
        Mystrians storage mystrian = mystrians[arrayId];
        mystrian.name = _name;
        mystrian.body = _body;
        mystrian.eye = _eye;
        mystrian.hair = _hair;
    ...
    function addNewMystrians(address player) public onlyOwner returns (uint256) {
        emit addMystrians(player);
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        string memory nameNumb = uint2str(mystriosRand(0x270F));
    ...
    function addChars(address _player,string memory _name,uint _color) public onlyOwner {
        charManager[_player].push(Char(_player,_name,_color));
        managers.push(_player);
    }
    
    function changeChars(address _player,string memory _name,uint _color) public onlyOwner {
        Char storage char = charManager[_player][0];
        char.name = _name;
        char.color = _color;
    }     
    ...
    function changeBody(uint256 tokenId, uint _body, uint _eye, uint _hair) public onlyOwner {
        uint arrayId = tokenId - 1;
        Mystrians storage mystrian = mystrians[arrayId];
        mystrian.body = _body;
        mystrian.eye = _eye;
    ...
    function getMystrianPrice() public view returns (uint) {
        return mystriumPrc;
    } 
    
    function changeMystrianPrice(uint _price) public onlyOwner {
        mystriumPrc = _price;
    }
    
    function getMystrianChgPrice() public view returns (uint) {
        return chngPrc;
    } 
    
    function changemystriosChgPrice(uint _price) public onlyOwner {
        chngPrc = _price;
    }
    ...
    function burnMystrian(uint256 tokenId) public onlyOwner {
        _burn(tokenId);
    }
    ...
    function getMystrian() external returns (bool){
        address senderAdr = msg.sender;
        ...
        require(allowance >= mystriumPrc, "Check the token allowance.");
        require(senderBalance >= mystriumPrc, "You have not enough balance.");
        bool transferData = tokenContract.transferFrom(senderAdr, contractAdd, mystriumPrc);
        require(transferData, "There is a problem about transfer.");
        addNewMystrians(senderAdr);
        return transferData;
    }
    
    function changeMystrian(uint256 tokenId, string memory _name,uint _body,uint _eye,uint _hair) external returns (bool) {
        address senderAdr = msg.sender;
        address contractAdd = address(this);
        uint256 senderBalance = tokenContract.balanceOf(senderAdr);
        tokenContract.approve(contractAdd,senderBalance);
        ...
        changeMystrians(tokenId,_name,_body,_eye,_hair);
        return transferData;
    }
    
    function trainingMystrian(uint256 tokenId) external returns (bool) {
        address senderAdr = msg.sender;
        address contractAdd = address(this);
        uint256 senderBalance = tokenContract.balanceOf(senderAdr);
        tokenContract.approve(contractAdd,senderBalance);
        ...
        require(transferData, "There is a problem about transfer.");
        require(ownerOf(tokenId) == senderAdr, "ERC721: Change of token that is not own");
        Mystrians storage mystrian = mystrians[tokenId];
        uint _atx = mystrian.atx + mystriosRand(ratio);
        uint _def = mystrian.def + mystriosRand(ratio);
        uint _str = mystrian.str + mystriosRand(ratio);
